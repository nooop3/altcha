...
import reactNativeQuickCrypto from 'react-native-quick-crypto';
...
const hashChallenge = async (salt: string, num: number, algorithm: string, cryptoPackage: string) => {
  ...
  switch (cryptoPackage) {
    ...
    case 'react-native-crypto':
      return reactNativeCrypto.createHash(algorithm.replace('-', '')).update(hashString).digest('hex')
    case 'react-native-quick-crypto':
      return reactNativeQuickCrypto.createHash(algorithm.replace('-', '')).update(hashString).digest('hex')
    default:
...
const AltChallenge = () => {
  ...
  const [cryptoPackageSelected, setCryptoPackageSelected] = useState(cryptoPackages[1]);
